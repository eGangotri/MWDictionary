...
import com.egangotri.vo.ClickLogMaster;
...
public class DictionaryDAOSupport extends HibernateDaoSupport
{
    ...
    {
        if(true) return;
        ClickLogMaster clickLogMaster = new ClickLogMaster();
        clickLogMaster.setModule(moduleName);
        clickLogMaster.setIpAddress(ipAddress);
        clickLogMaster.setTime(Calendar.getInstance());
        clickLogMaster.setSearchTerms(searchTerm);

        String country = "", city = "", region = "";
        try
        {
            LookupService lookupService = new LookupService(LookupService.GEOIP_DB_DIR + "GeoLiteCity.dat", LookupService.GEOIP_MEMORY_CACHE);
            ...
            clickLogMaster.setCountry(StringUtils.isNotBlank(country) ? country : "Unknown");
            clickLogMaster.setCity(StringUtils.isNotBlank(city) ? city : "Unknown");
            clickLogMaster.setRegion(StringUtils.isNotBlank(region) ? region : "Unknown");

            lookupService.close();

            getHibernateTemplate().save(clickLogMaster);
        }

        catch (Exception e)
        {
            Log.info("logUserActivityToDB", e);
        }
